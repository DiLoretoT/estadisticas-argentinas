...
def fetch_pobreza() -> dict[str, Any]:
  sources = load_sources()
  cfg = sources.get("pobreza", {})
  series_api = cfg.get("series_api", "")
  pobreza_id = cfg.get("tasa_pobreza_id", "")
  linea_indigencia_id = cfg.get("linea_indigencia_id", "")

  latest_pobreza, _ = _fetch_one(
    series_key="tasa_pobreza",
    display_name="Pobreza — personas bajo línea (%)",
    series_api=series_api,
    provider_series_id=pobreza_id,
    frequency="semiannual",
  )
  latest_linea_indigencia, _ = _fetch_one(
    series_key="linea_indigencia",
    display_name="Línea de indigencia (pesos corrientes)",
    series_api=series_api,
    provider_series_id=linea_indigencia_id,
    frequency="monthly",
  )
  ...
  if latest_linea_indigencia:
    payload["linea_indigencia"] = {
      "period": latest_linea_indigencia[0].strftime("%Y-%m-%d"),
      "value": round(latest_linea_indigencia[1], 2),
    }
  return payload


def main() -> None:
  payload = fetch_pobreza()
  if os.getenv("ETL_EXPORT_JSON", "1") == "1":
    write_json("pobreza.json", payload)
